...
auditlinerec () {
    ...
    if [ "$2" -eq "1" ]; then
      echo '<pre>'
       sudo /opt/butler_server/erts-7.3.1/bin/escript /home/gor/rpc_call.escript auditlinerec1 search_by "[[{'audit_id', 'equal', <<\"$1\">>}], 'key']."
       echo '</pre>'
    elif [ "$2" -eq "2" ]; then
      echo '<pre>'
       sudo /opt/butler_server/erts-7.3.1/bin/escript /home/gor/rpc_call.escript auditlinerec1 search_by "[[{'audit_id', 'equal', <<\"$1\">>}], 'record']."
       echo '</pre>'
    else
