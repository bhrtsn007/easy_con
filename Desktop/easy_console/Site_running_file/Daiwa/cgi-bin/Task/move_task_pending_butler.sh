...
move_task_pending_butler () {
    ...
    if [ "$2" -eq "1" ]; then
      echo '<pre>'
       sudo /opt/butler_server/erts-7.3.1/bin/escript /home/gor/rpc_call.escript movetaskrec search_by "[[{'butler_id','equal',$1},{'status', 'notequal', 'complete'}], 'key']."
       echo '</pre>'
    elif [ "$2" -eq "2" ]; then
      echo '<pre>'
       sudo /opt/butler_server/erts-7.3.1/bin/escript /home/gor/rpc_call.escript movetaskrec search_by "[[{'butler_id','equal',$1},{'status', 'notequal', 'complete'}], 'record']."
       echo '</pre>'
    else 
