...
all_pending_order () {
    ...
    if [ "$1" -eq "1" ]; then
       echo '<pre>'
       sudo /opt/butler_server/erts-7.3.1/bin/escript /home/gor/rpc_call.escript order_node search_by "[[{'status', 'equal','pending'}], 'key']."
       echo '</pre>'
    elif [ "$1" -eq "2" ]; then
      echo '<pre>'
       sudo /opt/butler_server/erts-7.3.1/bin/escript /home/gor/rpc_call.escript order_node search_by "[[{'status', 'equal', 'pending'}], 'record']."
       echo '</pre>'
    else 
