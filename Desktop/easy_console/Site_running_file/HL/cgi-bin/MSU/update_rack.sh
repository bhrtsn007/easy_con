...
Rack_position_update () {
    if [ "$4" -eq "1" ]; then
        ...
        barcode_validation=`sudo /opt/butler_server/erts-9.3/bin/escript /home/gor/rpc_call.escript gridinfo search_by "[[{'barcode', 'equal', \""$2"\"}], 'key']." | grep -oP '\[\K[^\]]+'`
        echo "<br>"
        ...
        then
            ...
            sudo /opt/butler_server/erts-9.3/bin/escript /home/gor/rpc_call.escript rackinfo update_rack_position_with_updates "[<<\"$1\">>,$3,\""$2"\", 'null']." 
        fi    
    else
