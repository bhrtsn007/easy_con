...
active_pps () {
    ...
    if [ "$1" -eq "1" ]; then
      echo '<pre>'
       sudo /opt/butler_server/erts-9.3/bin/escript /home/gor/rpc_call.escript ppsnode search_by "[[{'active_status', 'equal', 'true'}], 'key']."
       echo '</pre>'
    elif [ "$1" -eq "2" ]; then
      echo '<pre>'
       sudo /opt/butler_server/erts-9.3/bin/escript /home/gor/rpc_call.escript ppsnode search_by "[[{'active_status', 'equal', 'true'}], 'record']."
       echo '</pre>'
    else 
        echo "Wrong Choice"
    fi
